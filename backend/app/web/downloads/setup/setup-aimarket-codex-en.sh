...
usage() {
  cat <<'EOF'
Usage:
  bash setup-aimarket.sh [options]
...
Options:
  ...
  --api-key KEY              aimarket API key for setup mode
  ...
  --skip-endpoint-test       Do not test aimarket before writing config
...
msg() {
  ...
  if [[ "$LANG_CODE" == "ru" ]]; then
    case "$key" in
      title) printf '%s' 'Настройка приложений aimarket' ;;
      select_app) printf '%s' 'Выберите приложение:' ;;
      select_action) printf '%s' 'Выберите действие для {app}:' ;;
      configure) printf '%s' 'Настроить {app} на aimarket' ;;
      ...
      paste_key) printf '%s' 'Вставьте API ключ aimarket (ввод скрыт): ' ;;
      ...
      key) printf '%s' 'Ключ' ;;
      testing) printf '%s' 'Проверяю ключ aimarket' ;;
      ...
      restore_not_needed) printf '%s' 'Возврат endpoint для этого приложения не нужен. Чтобы перестать использовать aimarket, удалите custom provider/config в настройках приложения.' ;;
      ...
      cursor_not_managed) printf '%s' 'Управляемая настройка Cursor для aimarket не найдена — ничего не изменено.' ;;
      ...
      cursor_limit) printf '%s' 'Ограничение Cursor: Override OpenAI Base URL действует глобально и не перенаправляет Claude/Anthropic-модели. В некоторых версиях Cursor вложения с изображениями также обходят пользовательский OpenAI Base URL. Если картинка должна пройти через aimarket, используйте Codex, OpenCode или Hermes; Cursor с GPT/Grok используйте для текстовых задач в обычном режиме контекста.' ;;
      telemetry) printf '%s' 'Отправляю безопасное событие настройки в aimarket (приложение, ОС, действие, результат; API ключ используется только в Authorization header, чтобы определить ваш аккаунт).' ;;
      ...
      grok_command) printf '%s' "Используйте защищённый launcher 'grok-aimarket'. Он выбирает Grok 4.5 и передаёт API ключ только дочернему процессу Grok." ;;
      ...
      droid_config_hint) printf '%s' 'Droid использует локальный Factory settings.json, а API-ключ хранится в AIMARKET_API_KEY, не в JSON.' ;;
      *) printf '%s' "$key" ;;
    esac
  else
    case "$key" in
      title) printf '%s' 'aimarket app setup' ;;
      select_app) printf '%s' 'Select application:' ;;
      select_action) printf '%s' 'Select action for {app}:' ;;
      configure) printf '%s' 'Configure {app} for aimarket' ;;
      ...
      paste_key) printf '%s' 'Paste aimarket API key (input is hidden): ' ;;
      ...
      key) printf '%s' 'Key' ;;
      testing) printf '%s' 'Testing aimarket key' ;;
      ...
      restore_not_needed) printf '%s' 'Restore is not needed for this app. To stop using aimarket, remove the custom provider/config in the app settings.' ;;
      ...
      cursor_not_managed) printf '%s' 'No managed Cursor setup for aimarket was found; nothing was changed.' ;;
      ...
      cursor_limit) printf '%s' "Cursor limitation: Override OpenAI Base URL is global and does not reroute Claude/Anthropic models. Cursor can also bypass a custom OpenAI Base URL for image attachments in some versions. Use Codex, OpenCode, or Hermes when images must be sent through aimarket; use GPT/Grok models in Cursor for text-only normal-context work." ;;
      telemetry) printf '%s' 'Sending a safe setup event to aimarket (app, OS, action, result; the API key is used only in the Authorization header to identify your account).' ;;
      ...
      grok_command) printf '%s' "Use the secure launcher 'grok-aimarket'. It selects Grok 4.5 and loads the API key only into the Grok child process." ;;
      ...
      droid_config_hint) printf '%s' 'Droid uses the local Factory settings.json and keeps the API key in AIMARKET_API_KEY, never in JSON.' ;;
      *) printf '%s' "$key" ;;
    esac
  fi
}
  ...
  for attempt in 1 2; do
    if [[ -n "$body" ]]; then
      status="$(curl -sS -L -X "$method" "$url" \
        ...
        -H "User-Agent: aimarket-setup/$SCRIPT_VERSION" \
...
send_setup_telemetry() {
  ...
  if [[ "$APP" == "codex" ]]; then
    body+=",\"diagnostics\":$(codex_setup_diagnostics_json \"$ENDPOINT\" \"$MODEL\" \"AIMARKET_API_KEY\")"
  fi
...
backup_file() {
  local path="$1" backup
  if [[ -f "$path" ]]; then
    backup="$path.aimarket-backup-$(date +%Y%m%d-%H%M%S)"
    if [[ "$DRY_RUN" -eq 1 ]]; then
...
write_profile_block() {
  ...
  path="$(profile_path)"
  begin="# aimarket $app setup begin"
  end="# aimarket $app setup end"
  tmp="$(mktemp)"
...
codex_auth_diagnostics() {
  ...
  [[ -n "${AIMARKET_API_KEY:-}" ]] || fail "Codex auth diagnostic failed: AIMARKET_API_KEY is missing from the current process."
  [[ -n "$(printenv AIMARKET_API_KEY 2>/dev/null || true)" ]] || fail "Codex auth diagnostic failed: AIMARKET_API_KEY is missing from the current process."
  ok "Codex auth: provider 'aimarket' uses env_key 'AIMARKET_API_KEY' (present in the current process and shell profile)."
...
codex_runtime_probe() {
  ...
  probe_dir="$(mktemp -d 2>/dev/null || mktemp -d -t aimarket-codex-probe)"
...
assert_codex_provider_configuration() {
  ...
  while IFS= read -r profile; do
    ...
    if [[ -n "$profile_provider" && "$profile_provider" != "$provider" ]]; then
      warn "Profile '$(basename "$profile")' selects provider '$profile_provider' and overrides the aimarket default when used with 'codex --profile'."
    fi
  done < <(codex_profile_files)
}

codex_setup_diagnostics_json() {
  local requested_endpoint="${1:-$ENDPOINT}" requested_model="${2:-$MODEL}" env_key="${3:-AIMARKET_API_KEY}"
  ...
  while IFS= read -r profile; do
    ...
    [[ "$profile_provider" == "aimarket" || "$profile_provider" == "routercheap" ]] || other_count=$((other_count + 1))
  done < <(codex_profile_files)
...
codex_image_tool_root() {
  printf '%s/aimarket-image-tool' "${CODEX_HOME:-$HOME/.codex}"
}
...
install_codex_image_tool() {
  ...
  skill="${CODEX_HOME:-$HOME/.codex}/skills/aimarket-imagegen/SKILL.md"
...
remove_codex_image_tool() {
  local root skill
  root="$(codex_image_tool_root)"; skill="${CODEX_HOME:-$HOME/.codex}/skills/aimarket-imagegen"
  if [[ "$DRY_RUN" -eq 1 ]]; then
...
droid_display_name() {
  case "$1" in
    gpt-6-astra) printf '%s' 'aimarket GPT-6 Astra' ;;
    gpt-5.6-sol) printf '%s' 'aimarket GPT 5.6 Sol' ;;
    gpt-5.6-terra) printf '%s' 'aimarket GPT 5.6 Terra' ;;
    gpt-5.6-luna) printf '%s' 'aimarket GPT 5.6 Luna' ;;
    gpt-5.5) printf '%s' 'aimarket GPT 5.5' ;;
    *) printf 'aimarket %s' "$1" ;;
  esac
}
...
def is_managed_router_model(item):
    if item.get("provider") != "generic-chat-completion-api" or item.get("apiKey") != "${AIMARKET_API_KEY}":
        return False
...
const managed = { model, displayName: display, baseUrl: endpoint, apiKey: "${AIMARKET_API_KEY}", provider: "generic-chat-completion-api", maxOutputTokens: 16384 };
const managedHosts = new Set(["router.cheap", "direct.router-cheap.com"]);
function isManagedRouterModel(item) {
  if (item.provider !== "generic-chat-completion-api" || item.apiKey !== "${AIMARKET_API_KEY}") return false;
  let host = "";
...
write_codex_config() {
  ...
  if [[ -f "$path" ]]; then
    awk '
      $0 ~ /^[[:space:]]*\[model_providers.(aimarket|routercheap)(\.[^]]+)?\][[:space:]]*(#.*)?$/ { skip = 1; next }
      $0 ~ /^[[:space:]]*\[mcp_servers\.(aimarket_image|routercheap_image|relayfast_image|codexrodeo_image)(\.[^]]+)?\][[:space:]]*(#.*)?$/ { skip = 1; next }
  ...
  {
    cat "$tmp3"
    printf '\n[model_providers.aimarket]\n'
    printf 'name = "aimarket"\n'
    printf 'base_url = "%s"\n' "$ENDPOINT"
    printf 'env_key = "AIMARKET_API_KEY"\n'
    printf 'requires_openai_auth = false\n'
    printf 'wire_api = "responses"\n'
    printf '\n[mcp_servers.aimarket_image]\n'
    printf 'command = "%s"\n' "$command_escaped"
    printf 'args = ["%s"]\n' "$server_escaped"
    printf 'env_vars = ["AIMARKET_API_KEY"]\n'
    printf 'env = { CODEX_HOME = "%s", ROUTER_IMAGE_BASE_URL = "%s", ROUTER_IMAGE_API_KEY_ENV = "AIMARKET_API_KEY", ROUTER_IMAGE_MODEL = "gpt-image-2", ROUTER_IMAGE_BRAND = "aimarket" }\n' "${CODEX_HOME:-$HOME/.codex}" "$ENDPOINT"
...
restore_codex_config() {
  ...
  if [[ -f "$path" ]]; then
    awk '
      $0 == "[model_providers.aimarket]" || $0 == "[model_providers.routercheap]" { skip = 1; next }
      $0 == "[mcp_servers.aimarket_image]" || $0 == "[mcp_servers.routercheap_image]" { skip = 1; next }
      skip && $0 ~ /^\[/ { skip = 0 }
      !skip { print }
    ' "$path" > "$tmp1"
  else
...
render_grok_config() {
  ...
  if [[ -f "$source" ]]; then
    awk '
      /^[[:space:]]*\[model.(aimarket-grok-4-5|routercheap-grok-4-5)\][[:space:]]*(#.*)?$/ { skip = 1; next }
      skip && /^[[:space:]]*\[/ { skip = 0 }
      !skip { print }
    ' "$source" > "$cleaned"
  else
  ...
  {
    printf '\n[model.aimarket-grok-4-5]\n'
    printf 'model = "%s"\n' "$GROK_MODEL"
    printf 'base_url = "%s"\n' "$ENDPOINT"
    printf 'name = "Grok 4.5 via aimarket"\n'
    printf 'description = "Grok 4.5 through the aimarket OpenAI-compatible Responses API"\n'
    printf 'env_key = "AIMARKET_GROK_API_KEY"\n'
...
write_grok_config() {
  ...
  inspect_output="$(GROK_HOME="$validation_home" AIMARKET_GROK_API_KEY="$API_KEY" GROK_DISABLE_AUTOUPDATER=1 "$executable" inspect --json 2>&1)" || {
...
write_grok_launcher() {
  ...
  home="$(grok_home)"
  credential="$home/credentials/aimarket-api-key"
  backend="$home/credentials/aimarket-backend"
  launcher_dir="$HOME/.grok/bin"
  launcher="$launcher_dir/grok-aimarket"
  ...
  if command -v secret-tool >/dev/null 2>&1 && [[ -n "${DBUS_SESSION_BUS_ADDRESS:-}" ]]; then
    printf '%s' "$API_KEY" | secret-tool store --label='aimarket Grok Build API key' service aimarket application grok-build >/dev/null
    printf '%s\n' 'secret-tool' > "$backend"
    rm -f "$credential"
  else
...

  {
    ...
    printf 'BACKEND=%s\n' "$(shell_quote "$backend")"
    cat <<'EOF'
[[ -f "$BACKEND" ]] || { printf '[FAIL] aimarket Grok credential metadata is missing. Run setup again.\n' >&2; exit 1; }
case "$(cat "$BACKEND")" in
  secret-tool) API_KEY="$(secret-tool lookup service aimarket application grok-build || secret-tool lookup service router.cheap application grok-build || true)" ;;
  file) API_KEY="$(cat "$CREDENTIAL" 2>/dev/null || true)" ;;
  *) printf '[FAIL] Unknown aimarket Grok credential backend. Run setup again.\n' >&2; exit 1 ;;
esac
[[ -n "$API_KEY" ]] || { printf '[FAIL] aimarket Grok credential is empty. Run setup again.\n' >&2; exit 1; }
export GROK_HOME="$GROK_HOME_VALUE"
export AIMARKET_GROK_API_KEY="$API_KEY"
unset API_KEY
exec "$GROK_EXECUTABLE" -m aimarket-grok-4-5 "$@"
EOF
  } > "$launcher.tmp"
...
restore_grok_launcher() {
  ...
  if [[ -f "$backend" && "$(cat "$backend")" == "secret-tool" ]] && command -v secret-tool >/dev/null 2>&1; then
    secret-tool clear service aimarket application grok-build >/dev/null 2>&1 || true
    secret-tool clear service router.cheap application grok-build >/dev/null 2>&1 || true
  fi
  remove_profile_block grok-build
  if [[ "$DRY_RUN" -eq 1 ]]; then
    ok "Would remove aimarket Grok launcher and credentials."
  else
    rm -f "$backend" "$credential" "$HOME/.grok/bin/grok-aimarket" "$HOME/.grok/bin/grok-routercheap"
  fi
}
...
write_claude_settings() {
  ...
  state_path="$HOME/.aimarket/claude-code-cache-policy.json"
  tmp="$(mktemp)"
  state_tmp="$(mktemp)"
  error_file="$(mktemp)"
...
write_opencode_config() {
  ...
  path="$HOME/.config/opencode/aimarket.json"
  tmp="$(mktemp)"
  provider_tmp="$(mktemp)"
  models="$(available_chat_models)"
  [[ -n "$models" ]] || models="$MODEL"
  {
    ...
    printf '  "name": "aimarket",\n'
    printf '  "options": {"baseURL":"%s","apiKey":"{env:AIMARKET_OPENCODE_API_KEY}"},\n' "$(json_escape "$ENDPOINT")"
    printf '  "models": {\n'
    first=1
    while IFS= read -r model; do
      [[ -n "$model" ]] || continue
...
providers["aimarket"] = managed
data["provider"] = providers
data["$schema"] = "https://opencode.ai/config.json"
current_model = data.get("model")
if not isinstance(current_model, str) or not current_model.strip() or current_model.lower().startswith(("aimarket/", "routercheap/")):
    data["model"] = "aimarket/" + selected_model
if "enabled_providers" in data:
    ...
    if enabled == ["aimarket"]:
        ...
        data["enabled_providers"] = list(dict.fromkeys(enabled + ["aimarket"]))
print(json.dumps(data, ensure_ascii=False, indent=2))
PY
  else
    if command -v node >/dev/null 2>&1; then
...
providers.aimarket = JSON.parse(fs.readFileSync(providerPath, "utf8"));
data.provider = providers;
data["$schema"] = "https://opencode.ai/config.json";
if (typeof data.model !== "string" || !data.model.trim() || data.model.toLowerCase().startsWith("aimarket/") || String(data.model || "").toLowerCase().startsWith("routercheap/")) data.model = "aimarket/" + selectedModel;
if (Object.prototype.hasOwnProperty.call(data, "enabled_providers")) {
  ...
  if (data.enabled_providers.length === 1 && data.enabled_providers[0] === "aimarket") delete data.enabled_providers;
  else data.enabled_providers = [...new Set([...data.enabled_providers, "aimarket"])];
}
process.stdout.write(JSON.stringify(data, null, 2) + "\n");
JS
  fi
...
restore_opencode_config() {
  ...
  for item in "$path" "$HOME/.config/opencode/routercheap.json"; do
    if [[ -f "$item" ]]; then
      backup_file "$item"
      if [[ "$DRY_RUN" -eq 1 ]]; then
        ok "Would remove $item"
      else
        rm -f "$item"
      fi
    fi
...
hermes_managed_provider_name() {
  ...
  model="$(printf '%s' "$1" | tr '[:upper:]' '[:lower:]')"
  case "$model" in
    claude-*) printf '%s' aimarket-anthropic ;;
    gpt-*|o[1-9]|o[1-9]-*) printf '%s' aimarket ;;
    *) printf '%s' aimarket-compatible ;;
  esac
}
...
backup = path.with_name(path.name + ".aimarket-backup-" + datetime.now().strftime("%Y%m%d-%H%M%S") + "-" + secrets.token_hex(4))
shutil.copy2(path, backup)
temporary = path.with_name("." + path.name + ".aimarket-" + secrets.token_hex(6) + ".tmp")
try:
...
own = ("aimarket", "aimarket-anthropic", "aimarket-compatible")
...
if not restore:
    groups = {
        ...
        "aimarket-compatible": {
            "name": "aimarket (OpenAI compatible)",
            "base_url": endpoint,
            "key_env": "AIMARKET_API_KEY",
...
hermes_saved_api_key() {
  ...
  for path in "$active" "$legacy"; do
    ...
    [[ "$path" != "$seen" ]] || continue
    seen="$path"
    base="$(dotenv_value "$path" AIMARKET_BASE_URL)"
    [[ -n "$base" ]] || base="$(dotenv_value "$path" OPENAI_BASE_URL)"
    [[ "${base%/}" == "${ENDPOINT%/}" ]] || continue
    for key_name in AIMARKET_API_KEY OPENAI_API_KEY ANTHROPIC_API_KEY; do
...
remove_hermes_managed_blocks_at_path() {
  ...
  [[ -f "$path" ]] || return 0
  tmp="$(mktemp)"
  remove_marked_block "$path" "$tmp" "# aimarket setup begin" "# aimarket setup end"
...
write_hermes_env() {
  ...
  key_name="AIMARKET_API_KEY"
  tmp="$(mktemp)"
  block="$(mktemp)"
  {
    printf '%s=%s\n' "$key_name" "$API_KEY"
    printf 'AIMARKET_BASE_URL=%s\n' "$ENDPOINT"
  ...
  {
    cat "$tmp.other"
    printf '\n# aimarket setup begin\n'
    cat "$block"
    printf '# aimarket setup end\n'
  } > "$tmp.next"
  backup_file "$path"
  save_text_file "$path" "$tmp.next"
  ...
  if command -v hermes >/dev/null 2>&1; then
    if [[ "$DRY_RUN" -eq 1 ]]; then
      ...
      [[ "$saved_key" == "$API_KEY" ]] || fail "Hermes active credentials file does not contain the configured aimarket API key."
    fi
  else
...
backup_cursor_database() {
  local backup path
  backup="$CURSOR_DATABASE_PATH.aimarket-backup-$(date +%Y%m%d-%H%M%S)-$$"
  if [[ "$DRY_RUN" -eq 1 ]]; then
...
JS
  ...
  if ! result="$(AIMARKET_CURSOR_SETUP_KEY="$cursor_key" "$CURSOR_NODE_PATH" "$helper" "$CURSOR_SQLITE_MODULE_PATH" "$CURSOR_DATABASE_PATH" "$mode" "$ENDPOINT" aimarket "$models_file" 2>&1)"; then
    rm -f "$helper" "$models_file"
...
get_saved_api_key() {
  API_KEY_SOURCE=""
  env_value_to_api_key AIMARKET_API_KEY && return 0
  env_value_to_api_key AIMARKET_OPENCODE_API_KEY && return 0
...
setup_app() {
  ...
  case "$APP" in
    codex)
      ...
      tmp="$(mktemp)"
      printf 'export AIMARKET_API_KEY=%s\n' "$(shell_quote "$API_KEY")" > "$tmp"
      write_profile_block codex "$tmp"
      rm -f "$tmp"
      export AIMARKET_API_KEY="$API_KEY"
      ...
      if [[ "$DRY_RUN" -eq 1 ]]; then
        ...
        assert_codex_provider_configuration "$(codex_config_path)" "aimarket" "$ENDPOINT" "AIMARKET_API_KEY" "$MODEL"
      fi
    ...
    opencode)
      ...
      tmp="$(mktemp)"
      {
        printf 'export AIMARKET_OPENCODE_API_KEY=%s\n' "$(shell_quote "$API_KEY")"
        printf 'export OPENCODE_CONFIG=%s\n' "$(shell_quote "$HOME/.config/opencode/aimarket.json")"
      } > "$tmp"
